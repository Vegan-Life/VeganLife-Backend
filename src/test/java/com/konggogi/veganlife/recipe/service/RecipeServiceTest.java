...
public class RecipeServiceTest {
    ...
    void addTest() {
        ...
        Recipe recipe = recipeMapper.toEntity(request, member);

        given(memberQueryService.search(anyLong())).willReturn(member);

        recipeService.add(request, member.getId());
        ...
        assertThat(recipe.getRecipeImages()).hasSize(2);
        assertThat(recipe.getRecipeImages().stream().map(RecipeImage::getImageUrl))
                .containsAll(List.of("/image1.png", "/image2.png"));
